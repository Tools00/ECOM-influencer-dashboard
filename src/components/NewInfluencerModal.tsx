...
export function NewInfluencerButton() {
  ...
  return (
    <>
      ...
      {open && <NewInfluencerModal onClose={() => setOpen(false)} />}
    </>
  );
}

function NewInfluencerModal({ onClose }: { onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          <h2 className="text-sm font-semibold text-gray-800">Neuer Influencer</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Discount Code *">
                <input
                  ...
                  className={clsx(inputCls, "uppercase tracking-widest")}
                />
              </Field>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Kampagne">
                <input
                  ...
                  placeholder="Frühjahrskampagne Q1"
                  className={inputCls}
                />
              </Field>
          ...
          <fieldset className="space-y-3">
            ...
            <Field label="Typ *">
              <select
                ...
                onChange={(e) => set("comp_type", e.target.value as CompType)}
                className={inputCls}
              >
            ...
            {(form.comp_type === "fixed" || form.comp_type === "hybrid" || form.comp_type === "barter") && (
              <div className="grid grid-cols-2 gap-3">
                <Field label={form.comp_type === "barter" ? "Warenwert (€)" : "Fixum (€/Monat)"}>
                  <input
                    type="number"
                    min={0}
                    value={form.comp_fixed_eur}
                ...
                {form.comp_type !== "barter" && (
                  <Field label="Intervall">
                    <select
                      value={form.comp_interval}
                      onChange={(e) => set("comp_interval", e.target.value as FormState["comp_interval"])}
                      className={inputCls}
                    >
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
            ...
            >
              ...
              {saving ? "Wird gespeichert…" : "Influencer anlegen"}
            </button>
          </div>
        </form>
      </div>
...
function buildCompensation(form: FormState): Influencer["compensation"] {
  const base = {
    type: form.comp_type,
    start_date: form.contract_start_date || undefined,
  } as Influencer["compensation"];

  switch (form.comp_type) {
    case "fixed":
      return { ...base, type: "fixed", interval: form.comp_interval, fixed_eur: parseFloat(form.comp_fixed_eur) || 0 };
    case "commission":
      return { ...base, type: "commission", commission_pct: parseFloat(form.comp_commission_pct) || 0 };
    case "hybrid":
      return { ...base, type: "hybrid", interval: form.comp_interval, fixed_eur: parseFloat(form.comp_fixed_eur) || 0, commission_pct: parseFloat(form.comp_commission_pct) || 0 };
    case "per_post":
      return { ...base, type: "per_post", per_post_eur: parseFloat(form.comp_per_post_eur) || 0, posts_count: parseInt(form.comp_posts_count) || 1 };
    case "barter":
      return { ...base, type: "barter", fixed_eur: parseFloat(form.comp_fixed_eur) || 0 };
  }
}
